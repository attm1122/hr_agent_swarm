  ...
  implements EmployeeRepositoryPort
{
  ...
  async findAll(params: {
    ...
    return this.executeWithRetry(async () => {
      let query = this.table('employees')
        .select('*')
  ...
  async search(params: {
    ...
    return this.executeWithRetry(async () => {
      let dbQuery = this.table('employees')
        .select('*')
        .eq('tenant_id', params.tenantId)
  ...
  async save(employee: Employee, tenantId: string): Promise<void> {
    return this.executeWithRetry(async () => {
      const { error } = await this.table('employees')
  ...
  async update(id: string, data: Record<string, unknown>, tenantId: string): Promise<void> {
    return this.executeWithRetry(async () => {
      const { error } = await this.table('employees')
        .update(data)
        .eq('id', id)
