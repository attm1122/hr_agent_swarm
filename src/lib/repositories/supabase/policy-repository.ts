  ...
  implements PolicyRepositoryPort
{
  async findById(id: string, tenantId: string): Promise<PolicyDocument | null> {
    const { data, error } = await this.table('policy_documents')
      .select('*')
      .eq('id', id)
  ...
  async findAll(params: {
    ...
    let query = this.table('policy_documents')
      .select('*')
  ...
  async findChunks(params: {
    ...
    let query = this.table('policy_chunks')
      .select('*')
      .eq('tenant_id', params.tenantId);
  ...
  async searchContent(params: {
    ...
    const { data, error } = await this.table('policy_chunks')
      .select('*')
      .eq('tenant_id', params.tenantId)
